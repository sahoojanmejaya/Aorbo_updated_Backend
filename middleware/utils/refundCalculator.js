...
function calculateStandardRefund({ trekPrice, trekStartDatetime, cancellationTime, bookingStatus, policySettings = null }) {
    ...
    // Use second-precision integer comparison to avoid floating-point drift
    const timeDiffSeconds = Math.floor(
        (trekStartDatetime.getTime() - cancellationTime.getTime()) / 1000
    );
    const HOURS_72 = 72 * 3600;
    const HOURS_48 = 48 * 3600;
    const HOURS_24 = 24 * 3600;

    // Boundary rules:
    //   >72h          → 20%  (strictly more than 72h)
    //   exactly 72h   → 50%
    //   ≥48h and <72h → 50%  (timeDiffSeconds in [48h, 72h))
    //   exactly 48h   → 70%
    //   ≥24h and <48h → 70%  (timeDiffSeconds in [24h, 48h))
    //   exactly 24h   → 100%
    //   <24h or ≤0    → 100%
    let deductionPercent;
    let slabInfo;

    if (timeDiffSeconds > HOURS_72) {
        deductionPercent = settings72hPlus;
        slabInfo = ">72h before trek";
    } else if (timeDiffSeconds >= HOURS_48) {
        // covers exactly 72h and the [48h, 72h] band
        deductionPercent = settings48_72h;
        slabInfo = "48-72h before trek";
    } else if (timeDiffSeconds >= HOURS_24) {
        // covers exactly 48h and the [24h, 48h) band
        deductionPercent = settings24_48h;
        slabInfo = "24-48h before trek";
    } else if (timeDiffSeconds > 0) {
        // <24h (including exactly 24h already handled above)
    ...
    const timeRemainingHours = Math.max(0, timeDiffSeconds / 3600);
...
async function getPolicySettings(models, policyType) {
    try {
        ...
        // Caller should handle null return and fall back to defaults
        return null;
    }
}
...
function calculateFlexibleRefundV2({
    ...
    // Input logged at service layer if debug is needed
