...
exports.getTbrById = async (req, res) => {
    try {
        ...
        if (tbr.trek) {
            ...
            const [inc, exc, act, itin, acc, stg] = await Promise.all([
                inclusionIds.length > 0 ? Inclusion.findAll({ where: { id: { [Op.in]: inclusionIds } }, attributes: ["id", "name"] }) : [],
                exclusionIds.length > 0 ? Exclusion.findAll({ where: { id: { [Op.in]: exclusionIds } }, attributes: ["id", "name"] }) : [],
                ...
                Accommodation.findAll({ where: { trek_id: tbr.trek.id } }),
                TrekStage.findAll({ where: { trek_id: tbr.trek.id }, include: [{ model: City, as: "city", attributes: ["id", "cityName"] }], order: [["date_time", "ASC"]] })
            ]);
            inclusions_data = inc;
            exclusions_data = exc;
            activities_data = act;
        ...
        const tbrData = {
            ...
            operator: vendor ? {
                ...
                logo: vendor.business_logo || null,
                email: vendor.company_info?.company_email || vendor.email || 'N/A',
                phone: vendor.company_info?.company_phone || vendor.phone || 'N/A',
                captainName: vendor.business_name + ' Team', // Mock default unless trek specifies captain
                captainPhone: vendor.company_info?.company_phone || 'N/A',
                credibilityScore: 5
            } : { id: null, name: 'Unknown Vendor', logo: null },
            ...
            inclusions: Array.from(new Set(inclusions_data.map(i => i.name))),
            exclusions: Array.from(new Set(exclusions_data.map(e => e.name))),
            ...
            trekStages: trek_stages.map((stage) => ({
                id: stage.id,
