...
exports.updateTrek = async (req, res) => {
  try {
    ...
    if (updateData.trek_status === "approved") {
      updateData.status = "active";
      updateData.approved_at = new Date();
      updateData.approved_by = req.user?.id || null;
      updateData.rejected_reason = null;
    } else if (updateData.trek_status === "rejected") {
      updateData.status = "deactive";
      // rejected_reason must be set by caller via req.body.rejected_reason
    }
    ...
    if (updateData.trek_status) {
      try {
        const { TrekAuditLog } = require("../../models");
        await TrekAuditLog.create({
          vendor_id: req.user?.id || null,
          trek_id: trek.id,
          current_status: updateData.trek_status,
          target_entity: `${trek.title}-${trek.mtr_id}`,
          details: `Trek status set to '${updateData.trek_status}' by admin`,
        });
      } catch (auditErr) {
