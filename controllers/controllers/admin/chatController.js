...
const getChatById = async (req, res) => {
  try {
    ...
    messages.forEach(msg => {
      if (msg.sender_id !== id) chatUserIds.add(msg.sender_id);
      if (msg.receiver_id !== id) chatUserIds.add(msg.receiver_id);
    });
    ...
    const chatList = Array.from(chatUserIds)
      ...
      .map(chatUserId => {
        ...
        const recentMessage = messages.find(
          msg =>
            (msg.sender_id === chatUserId && msg.receiver_id === id) ||
            (msg.receiver_id === chatUserId && msg.sender_id === id)
        );
