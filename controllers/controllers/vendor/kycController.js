...
// BUG #4 FIX: Removed hardcoded JWT_SECRET fallback — fail fast at startup if env var is missing
...
const verifyTempToken = (req, res, next) => {
    try {
        ...
        // BUG #10 FIX: Removed static-temp-token bypass — was allowing unauthenticated access to all KYC endpoints
        // Only valid JWT tokens are accepted now
...
const uploadDocuments = (req, res, next) => {
    ...
    upload.fields([
        ...
        if (err) {
            logger.error(`[KYC_UPLOAD] Multer error: ${err.message}`, {
                error: err,
                body: req.body,
                userId: req.user?.id
            });
        ...
        logger.info(`[KYC_UPLOAD] Files processed successfully`, {
            userId: req.user?.id,
            filesCount: filesReceived.length,
            files: filesReceived,
            bodyKeys: Object.keys(req.body)
        });

        if (req.files && req.files.business_logo) {
            logger.info(`[KYC_UPLOAD] Received business_logo: ${req.files.business_logo[0].filename}`, {
                mimetype: req.files.business_logo[0].mimetype,
                size: req.files.business_logo[0].size
            });
        }
...
// BUG #8 FIX: Password is now cryptographically random — was previously derived from vendor's full name (predictable and insecure)
// BUG #8 FIX: Import crypto at top of file if not already imported
const crypto = require('crypto');
const generatePassword = () => {
    // BUG #8 FIX: 12 random bytes = 24 character hex string, completely unpredictable
...
exports.savePersonalBusinessStep = async (req, res) => {
    ...
    // BUG #7 FIX: Uncommented try block — was commented out causing unhandled promise rejections and Node crashes
    try {
        ...
        logger.info(`[KYC_STEP1] Starting savePersonalBusinessStep`, { 
        ...
        if (existingVendor) {
            ...
            // BUG #8 FIX: generatePassword no longer takes fullName argument — password is now random
            const password = generatePassword();
            const hashedPassword = await bcrypt.hash(password, 10);

            // BUG #8 FIX: [DEV ONLY] Password logged for development. Remove before production. Send via email instead.
            console.log("[DEV ONLY - REMOVE IN PROD] Generated random password for vendor (send via email):", password);
            ...
            logger.info(`[KYC_STEP1] Personal & Business details updated successfully`, {
                vendorId: existingVendor.id,
                kyc_step: 2
            });
        ...
        // BUG #7 FIX: Uncommented catch block — Node process no longer crashes on DB errors in KYC Step 1
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: "Something went wrong saving KYC data",
...
exports.saveDocumentsStep = async (req, res) => {
    ...
    try {
        ...
        logger.info(`[KYC_STEP2] Received document upload request`, {
            userId: req.user?.id,
        ...
        if (files && Object.keys(files).length > 0) {
            ...
            const documentFieldMapping = {
                ...
                // BUG #14 FIX: Fixed typo in field mapping key
            ...
            Object.keys(files).forEach(fieldName => {
                ...
                if (files[fieldName] && files[fieldName][0]) {
                    ...
                    if (columnName) {
                        ...
                        // Store relative path (forward-slash) so frontend URL construction works
                        documents[columnName] = `storage/documents/${file.filename}`;
                        console.log(`Storing ${fieldName} path in ${columnName}: storage/documents/${file.filename}`);
                    }
                }
            });
...
exports.saveBankDetailsStep = async (req, res) => {
    try {
        ...
        // BUG #9 FIX: Now finds vendor by phone number from request — was using ORDER BY id DESC which mixed up data between concurrent vendors
        const phoneFromRequest = req.body.phone || req.user?.phone;
        if (!phoneFromRequest) {
            return res.status(400).json({ success: false, message: "Phone number required to identify vendor" });
        }
        const normalizePhone = (phone) => {
            const digits = phone.replace(/\D/g, '');
            return digits.slice(-10);
        };
        const normalizedPhone = normalizePhone(phoneFromRequest);
        const latestVendor = await Vendor.findOne({
            include: [{
                model: User,
                as: "user",
        ...
        if (!latestVendor) {
            return res.status(404).json({ success: false, message: "Vendor not found for this phone number" });
        }
...
exports.submitKYC = async (req, res) => {
    try {
        ...
        console.log(`[${timestamp}] KYC Step 4 - KYC submission received`);
        
        logger.info(`[KYC_SUBMIT] Received finalize request`, {
            userId: req.user?.id,
            body: req.body
        });
        ...
        if (latestVendor) {
            ...
            res.json({
                ...
                documents: {
                    ...
                    // BUG #14 FIX: Fixed typo in response field name
                    business_logo: latestVendor.business_logo ? {
            ...
            // BUG #23 FIX: Return proper not_found status instead of fake temp_vendor_id data — fake data was causing KycOnboarding to behave incorrectly
            return res.status(200).json({
                success: false,
                kyc_status: "not_found",
                message: "No vendor found for this phone number"
            });
        }
    } catch (error) {
...
// BUG #12 FIX: Deleted testDatabase and testFileUpload controller functions — were exposing DB info and allowing arbitrary file uploads without auth
