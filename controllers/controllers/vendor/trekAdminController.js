...
exports.getVendorTreks = async (req, res) => {
    try {
        const vendorUserId = req.user?.id;
        const { trek_status, name, page = 1, limit = 10 } = req.query;
        const parsedPage = Math.max(parseInt(page) || 1, 1);
        const parsedLimit = Math.min(parseInt(limit) || 10, 50);
        const parsedOffset = (parsedPage - 1) * parsedLimit;

        // Allow both vendors and admins to use this endpoint
        // Admins won't have a vendor account but shouldn't be blocked
        const isAdmin = req.user?.role === 'admin' || req.user?.roleId === 1;

        if (!vendorUserId && !isAdmin) {
            return res.status(403).json({
                success: false,
                message: "Access denied",
            });
        ...
        // Only filter by vendor_id if NOT an admin
        if (!isAdmin) {
            ...
            include: [
                {
                    model: Vendor,
                    as: "vendor",
                    attributes: { exclude: ["user_id"] }, // Previously restricted to ['id', 'business_name', 'status'] which broke Approval UI
                    include: [
                        {
                            model: User,
                            as: "user",
...
exports.updateTrekStatus = async (req, res) => {
    try {
        // Accept both reject_reason (frontend) and rejected_reason (legacy) field names
        const { id, status, reject_reason, rejected_reason } = req.body;
        const rejectionReason = reject_reason || rejected_reason || null;
        // Use user_id (users table FK) if present (vendor tokens), else fall back to id (admin tokens)
        ...
        // Prepare update data
        const updateData = { trek_status: status };
        ...
        try {
            await TrekAuditLog.create({
                ...
                trek_id: trek.id,
                current_status: status,
                target_entity: `${trek.title}-${trek.mtr_id}`,
                details: `Trek status changed from '${oldStatus}' to '${status}'${status === "rejected" && rejectionReason ? `. Reason: ${rejectionReason}` : ""
                    }`,
            });
        } catch (auditErr) {
            logger.error("Audit log creation failed:", auditErr.message);
        }

        return res.status(200).json({
            success: true,
            message: "Trek status updated successfully",
            data: { id: trek.id, trek_status: status },
        });
