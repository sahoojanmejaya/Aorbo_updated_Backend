...
exports.getVendorBookings = async (req, res) => {
    try {
        ...
        const bookingsData = bookings.map((booking) => {
            ...
            // Calculate trek_status based on batch dates
            let trek_status = "upcoming";
            if (data.status === "cancelled") {
                trek_status = "cancelled";
            } else if (data.batch) {
                const now = new Date();
                const batchStart = new Date(data.batch.start_date);
                const batchEnd = data.batch.end_date ? new Date(data.batch.end_date) : null;
                if (batchEnd && now > batchEnd) {
                    trek_status = "completed";
                } else if (batchStart <= now && (!batchEnd || now <= batchEnd)) {
                    trek_status = "ongoing";
                } else {
                    trek_status = "upcoming";
                }
            }
            data.trek_status = trek_status;
