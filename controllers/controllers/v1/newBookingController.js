...
exports.confirmCancellation = async (req, res) => {
    ...
    try {
        ...
        // Server-side refund calculation — never trust client-supplied amount
        const preview = await cancellationService.previewCancellation(booking_id);
        if (preview.status !== 200) {
            await transaction.rollback();
            return res.status(preview.status).json({ success: false, message: preview.error });
        }

        const totalRefundableAmount = roundHalfUp(preview.refund_amount);
        const deductionAmount = roundHalfUp(preview.deduction_amount);
        const deductionAdmin = deductionAmount;
        const deductionVendor = deductionAmount;
