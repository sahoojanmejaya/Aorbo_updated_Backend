...
const getBookingDisputeStatus = async (req, res) => {
    try {
        ...
        // Find ALL disputes by booking ID (ownership already verified above)
        ...
        logger.error("dispute", "Error fetching dispute status", { error: error.message });
        res.status(500).json({ success: false, message: "Failed to fetch dispute information" });
    }
};
...
const updateBookingDisputeStatus = async (req, res) => {
    try {
        ...
        const ownerBooking = await Booking.findOne({ where: { id: bookingId, customer_id: customerId } });
        if (!ownerBooking) {
            return res.status(404).json({ success: false, message: "Booking not found" });
        }

        // Find dispute by booking ID owned by this customer
        const dispute = await IssueReport.findOne({
            where: { booking_id: bookingId, customer_id: customerId },
            include: [{ model: Booking, as: "booking", attributes: ['id', 'status', 'final_amount'] }]
        });

        if (!dispute) {
            return res.status(404).json({ success: false, message: "No dispute found for this booking" });
        }
        ...
        logger.error("dispute", "Error updating dispute status", { error: error.message });
