...
const validateTrek = [
    ...
    // BUG FIX: Added required field validation for all red star (*)
    // marked fields in the Trek Creation form. Form submission is
    // now blocked if any mandatory red star field is left empty.
    // Validation applied at frontend field level, validateTrekPayload
    // level, and backend route validator level.
    body("captain_id")
        .notEmpty()
        .withMessage("Trek Captain is required")
        .isInt({ min: 1 })
    ...
    body("city_ids")
        .notEmpty()
        .withMessage("Trek Cities are required")
        .isArray({ min: 1 })
        .withMessage("At least one city must be selected")
        .custom((value) => {
    ...
    // BUG FIX: Restricted trek duration (number of days) input to
    // a maximum of 2 digits (1–99). Input is now locked to prevent
    // entering 3-digit numbers (100 and above). Validation applied
    // at both input level and form submission level.
    body("duration_days")
        .notEmpty()
        .withMessage("Duration days is required")
        .isInt({ min: 1, max: 99 })
        .withMessage("Duration days must be between 1 and 99"),
    ...
    // BUG FIX: Restricted Price per Person (₹) input to a maximum
    // of 5 digits (1–99999). The 6th digit is now blocked at keypress
    // level. Decimal/float values are not allowed — only integers
    // are accepted. Validation applied at input, onChange, form
    // submission, and backend levels.
    body("base_price")
        .notEmpty()
        .withMessage("Base price is required")
        .isInt({ min: 1, max: 99999 })
        .withMessage("Base price must be a whole number between 1 and 99999"),

    // BUG FIX: Restricted Total Group Size input to a maximum
    // of 2 digits (0–99). The 3rd digit is now blocked at keypress
    // level. Values of 100 and above are rejected. Validation
    // applied at input, onChange, form submission, and backend levels.
    body("max_participants")
        .notEmpty()
        .withMessage("Maximum participants is required")
        .isInt({ min: 0, max: 99 })
        .withMessage("Total group size must be between 0 and 99"),
    ...
    // BUG FIX: Added validation to ensure Exclusions field is
    // required before trek creation is allowed. Trek creation is
    // now blocked if Exclusions are empty.
    body("exclusions")
        .notEmpty()
        .withMessage("Please add at least one Exclusion before creating the trek.")
        .isArray({ min: 1 })
        .withMessage("Please add at least one Exclusion before creating the trek."),
...
const validateLimitedTrekUpdate = [
    ...
    // BUG FIX: Added validation to ensure accommodations field is
    // required before trek creation is allowed. Trek creation is
    // now blocked if accommodations are empty.
    body("accommodations")
        .notEmpty()
        .withMessage("Accommodations are required")
        .isArray({ min: 1 })
        .withMessage("Accommodations must be a non-empty array"),
